...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
  ...
  float Fz = -momentCmd.z / l;

  // float F4 = (Fc + Fx - Fy - Fz) / 4;
  // float F3 = (Fz - Fx) / 2 + F4;
  // float F2 = (Fc - Fx) / 2 - F3;
  // float F1 = Fc - F2 - F3 - F4;

  float F1 = (Fc + Fx + Fy + Fz) / 4;
  float F2 = (Fc - Fx + Fy - Fz) / 4;
  float F3 = (Fc + Fx - Fy - Fz) / 4;
  float F4 = (Fc - Fx - Fy + Fz) / 4;
...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  V3F b = V3F(bxa, bya, 0.f);
  V3F bc = V3F(accelCmd.x / -c, accelCmd.y / -c, 0.f);
  bc.constrain(-sin(maxTiltAngle), sin(maxTiltAngle));
  V3F bp = kpBank * (bc - b);

  pqrCmd.x = (R21 * bp.x - R11 * bp.y) / R33;
  pqrCmd.y = (R22 * bp.x - R12 * bp.y) / R33;
  pqrCmd.z = 0.f;
...
float QuadControl::AltitudeControl(float posZCmd, float velZCmd, float posZ, float velZ, Quaternion<float> attitude, float accelZCmd, float dt)
{
  ...
  accelZCmd += kpVelZ * velZ_err + KiPosZ * integratedAltitudeError;

  thrust = ((float)CONST_GRAVITY - accelZCmd) / bz * mass;
